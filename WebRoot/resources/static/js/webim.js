	...
	for (var i = 0; i < rosterMsg.length; i++) {
		...
		if (contact.subscription == 'both') {
			...
			if (!isexist) {
				...
				$('<img>').attr({
					"src" : basePath+"/resources/static/img/head/contact_normal.png"
				}).appendTo(lielem);
...
var buildContactDiv = function(contactlistDivId, roster) {
	...
	for (i = 0; i < roster.length; i++) {
		...
		$('<img>').attr("src", basePath+"/resources/static/img/head/contact_normal.png").appendTo(
				lielem);
...
var buildListRoomDiv = function(contactlistDivId, rooms, type) {
	...
	for (i = 0; i < rooms.length; i++) {
		...
		$('<img>').attr({
			'src' : basePath+'/resources/static/img/head/group_normal.png'
		}).appendTo(lielem);
...
var createMomogrouplistUL = function createMomogrouplistUL(who, message) {
	...
	imgelem.setAttribute("src", basePath+"/resources/static/img/head/contact_normal.png");
...
Easemob.im.EMOTIONS = {
    path: basePath+'/resources/static/img/faces/'
    , map: {
        '[):]': 'ee_1.png',
        '[:D]': 'ee_2.png',
        '[;)]': 'ee_3.png',
